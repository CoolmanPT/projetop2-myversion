...
import pt.ipleiria.estg.ei.p2.blast.modelo.suportados.Reagente;
...
public class BaseSuportadora extends Base implements SensivelOndaChoque, Iteravel,
        ReagenteFoguete, Interativo, Reagente {
    ...
    public void reagirInteracao() {
        if (podeInteragir()) {
             if(suportado instanceof Reagente){
                ((Reagente) suportado).reagirInteracao();
            }
            else {
    ...
    public boolean podeInteragir() {
        if ((suportado instanceof Reagente && ((Reagente) suportado).podeInteragir()) || (suportado instanceof SuportadoAgrupavel && ((SuportadoAgrupavel) suportado).podeInteragir())){
            return true;
        }
        return false;
